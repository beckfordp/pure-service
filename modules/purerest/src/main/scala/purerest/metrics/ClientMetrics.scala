...
object ClientMetrics {
  ...
  /** Wraps a `Client[F]` with a `http.client.request.duration` histogram
    ...
    * server address, and response status code (OTel semantic-convention attribute
    * names).
    */
  def middleware[F[_]: Async](meter: Meter[F])(client: Client[F]): Client[F] =
    Client[F] { req =>
      for {
        histogram <- Resource.eval(meter.histogram[Double]("http.client.request.duration").withUnit("s").create)
        ...
        _ <- Resource.eval(
          histogram.record(
            ...
            List(
              ...
              Attribute("server.address", req.uri.authority.map(_.host.value).getOrElse("")),
              Attribute("http.response.status_code", response.status.code.toLong)
            )
          )
        )
      } yield response
