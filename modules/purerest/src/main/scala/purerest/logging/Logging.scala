...
object Logging {
  ...
  /** A trace-correlated structured logger backed by SLF4J, for `Main`/production use. */
  def create[F[_]: Sync](tracer: Tracer[F], name: String): F[StructuredLogger[F]] =
  ...
  /** Wraps `underlying` so every log call's context is stamped with the current
    * span's trace id and span id, if a span is active. Generic over the underlying
    * logger so it's directly testable with any `StructuredLogger[F]`.
    */
  def traceCorrelated[F[_]: Sync](tracer: Tracer[F], underlying: StructuredLogger[F]): StructuredLogger[F] =
    new StructuredLogger[F] {
      ...
      def trace(message: => String): F[Unit] = withTrace(Map.empty).flatMap(underlying.trace(_)(message))
      ...
      def debug(message: => String): F[Unit] = withTrace(Map.empty).flatMap(underlying.debug(_)(message))
      ...
      def info(message: => String): F[Unit] = withTrace(Map.empty).flatMap(underlying.info(_)(message))
      ...
      def warn(message: => String): F[Unit] = withTrace(Map.empty).flatMap(underlying.warn(_)(message))
      ...
      def error(message: => String): F[Unit] = withTrace(Map.empty).flatMap(underlying.error(_)(message))
