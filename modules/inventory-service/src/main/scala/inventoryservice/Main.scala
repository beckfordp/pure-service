...
object Main extends IOApp.Simple {
  ...
  private val inducedFailure: InducedFailureConfig =
    InducedFailureConfig(
      failureRate = sys.env.get("INVENTORY_INDUCED_FAILURE_RATE").flatMap(_.toDoubleOption).getOrElse(0.0),
      delay = sys.env
    ...
    Tracing.console[IO]("inventory-service").use { tracer =>
      for {
        ...
        docsRoutes = Docs.routes[IO](
          "Inventory Service",
          "1.0",
          List(InventoryRoutes.serverEndpoint[IO](store, logger, inducedFailure))
        )
