...
trait OrderStore[F[_]] {
  def create(item: String, quantity: Int, reservationId: String, reservedQuantity: Int): F[Order]
...
object OrderStore {
      ...
      new OrderStore[F] {
        def create(item: String, quantity: Int, reservationId: String, reservedQuantity: Int): F[Order] =
          for {
            ...
            order = Order(id, item, quantity, defaultStatus, reservationId, reservedQuantity, now)
  ...
  private val selectOrder: skunk.Query[UUID, (String, Int, String, UUID, Int, OffsetDateTime)] =
    sql"""
          ...
          def create(item: String, quantity: Int, reservationId: String, reservedQuantity: Int): F[Order] =
            pool.use { session =>
              for {
                ...
                createdAt <- session
                  .prepare(insertOrder)
                  .flatMap(_.unique((id, item, quantity, reservationUuid, reservedQuantity)))
              } yield Order(
                id.toString,
                item,
                quantity,
          ...
          def get(id: String): F[Option[Order]] =
            pool.use { session =>
              for {
                ...
                case (item, quantity, status, reservationUuid, reservedQuantity, createdAt) =>
                  Order(id, item, quantity, status, reservationUuid.toString, reservedQuantity, createdAt.toInstant)
              }
            }
        }
      }
