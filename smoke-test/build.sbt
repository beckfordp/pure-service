...
// root pure-service build.sbt, so `sbt projects`/`sbt compile`/`sbt test` at the
...
// libraryDependencies entry, not the internal .dependsOn(purerest) project
...
val purerestVersion = sys.props.getOrElse(
  ...
  sys.error(
    "purerestVersion system property not set — run via " +
      ...
      "(the output of `sbt purerest/version` in the main repo, after " +
      "`sbt purerest/publishLocal`)."
  )
)
...
lazy val root = project
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      "io.github.beckfordp" %% "purerest" % purerestVersion,
