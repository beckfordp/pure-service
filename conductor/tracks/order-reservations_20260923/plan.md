...
- [ ] Task: Write a failing test asserting the `orders` table has `reservation_id`, `reserved_quantity`, `status` columns after migration (Testcontainers Postgres, extends the pattern from `MigrationsSuite`) (Red).
- [ ] Task: Add `V2__add_reservation_to_orders.sql`; extend `Order` with `status`, `reservationId`, `reservedQuantity`, `createdAt: java.time.Instant` (Green).
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Schema & Domain Model' (Protocol in workflow.md)
